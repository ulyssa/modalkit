    ...
    I: ApplicationInfo,
{
    ...
    fn _highlight_ranges(
        ...
        let hinfo = self.buffer.selection_intervals(self.gid);
        let mut ranges = vec![];
    ...
    pub fn get_trim(&self) -> EditRope {
        self.buffer.get().trim_end_matches(super::is_newline)
    }
    ...
    pub fn line_leftover(&mut self, dir: MoveDir1D, count: usize) -> usize {
        let leader = self.buffer.get_leader(self.gid);
        let text = self.buffer.get();

        match dir {
            MoveDir1D::Next => {
                let avail = text.get_lines().saturating_sub(1).saturating_sub(leader.y);
...
mod tests {
    ...
    fn strs(hist: &HistoryList<EditRope>) -> Vec<String> {
        hist.iter().map(ToString::to_string).collect()
    }
